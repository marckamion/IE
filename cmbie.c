...
int main(int argc,char *argv[])
{
  ...
  for(k=1.0/tau0; k<= 3001.00001/tau0; k+=30.0/tau0) {
    ...
    taustep = 0.25;      /*  size of conformal-time steps  */
	...
	if(tau > tau_handoff - 20.0) {
	  ...
	  R=0.75 * Omega_b/Omega_ph * a * a_eq;    /* note that this is the inverse of MB's and the CLASS paper's  R   */
	  alphadot[count] = hdot[count] - 3.0 * sqr(calHeq)/a/a* (Omega_ph/Omega_r)*((y[3] + R * y[6]) + fnu  * y[8]); 
	}
	Pie=1.25*sigma;      /*   this is the value of Pi in the TCA   */
      }
	...
	kappadotprime[count+1]=kappadot;
	  alphadot[count+1] = hdot[count+1] - 3.0 * sqr(calHeq)/a/a* (Omega_ph/Omega_r)*((y[3] + R * y[6]) + fnu  * y[8]);
	/*    now do the integral equations   */
	/*  initialize Delta2 and Pi   */
	Delta2[count+1]=0.0;
	Pi[count+1]=0.0;

	/*   test1, test2, and test3 are used to test the integration  */
	/* test1 = 0.0;
	  test2 = 0.0;
	  test3 = 0.0;  */
	for(i=2;i<=count;i++) {
	  ...
	  /*  Kip1, Ki, Kim1 weight the contributions of the integrands at i+1, i, and i=1, respectively  */
	  if(Deltakappa>0.01) {
	    edk = exp(-Deltakappa);
	  ...
	  /*   these are an alternative weightings  */
	  ...
	  /*  Iip1, Ii, Iim1 are the Delta2 integrands at i+1, i, and i=1  
	      and Piip1, Pii, and Piim1 are similar for Pi  */
	...
	/*   here we invert the 2x2 matrix associated with the semi-implicit integration scheme;  the numerical details come from the specific forms of the integrands as well as the limits of the radial eigenfuctions when their arguments vanish  */
	sum = Pi[count+1]+Delta2[count+1];
	Pi[count+1] = sum /(1.0-(7.0/10.0)*Kip1);
	Delta2[count+1] += 0.1*Pi[count+1]*Kip1;

	sigma = 2.0*Delta2[count+1];     /*   this is needed for tcaderivs   */
	Pie=Pi[count+1];
	count++;       /*   advance the count  */
      ...
      /*  if the integrations are working, test1, test2, and test3 should all be 1.0  */
...
/*    differential equations for tight-coupling approximation  */
{
  ...
  if(kappadot/R>10.0*cH && t<tau_handoff+taustep) {
    ...
    /*  and here's the TCA approx for thetadot  */
